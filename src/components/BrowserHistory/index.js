...
class BrowserHistory extends Component {
  ...
  deleteTodo = id => {
    ...
    const filteredList = browserHistory.filter(eachTodos => eachTodos.id !== id)

    this.setState({browserHistory: filteredList})
  }

  render() {
    const {searchInput, browserHistory} = this.state

    const searchResults = browserHistory.filter(eachTodos =>
      eachTodos.title.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
      <div>
